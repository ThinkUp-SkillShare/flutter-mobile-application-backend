...
public class StudyGroupController : ControllerBase
{
    ...
    public async Task<IActionResult> UpdateStudyGroup(int id, [FromBody] UpdateStudyGroupDto dto)
    {
        try
        {
            ...
            if (!await _groupManagementService.CanUserEditGroup(id, userId)) return Forbid();
    ...
    public async Task<IActionResult> DeleteStudyGroup(int id)
    {
        try
        {
            ...
            if (!await _groupManagementService.CanUserDeleteGroup(id, userId)) return Forbid();
    ...
    public async Task<IActionResult> JoinGroup(int id)
    {
        try
        {
            ...
            if (existingMembership != null) return BadRequest(new { message = "Already a member of this group" });
    ...
    public async Task<IActionResult> LeaveGroup(int id)
    {
        try
        {
            ...
            if (group != null && group.CreatedBy == userId)
            {
                ...
                if (adminCount <= 1) return BadRequest(new { message = "Cannot leave: you are the only admin" });
            }
    ...
    public async Task<ActionResult<GroupPermissionsDto>> GetUserPermissions(int id)
    {
        try
        {
            ...
            _logger.LogInformation("Permissions for user {UserId} in group {GroupId}: {@Permissions}", userId, id,
                permissions);
    ...
    public async Task<IActionResult> PromoteToAdmin(int id, int memberId)
    {
        try
        {
            ...
            if (!result)
                ...
                {
                    message =
                        "Failed to promote member. You may lack permissions or the member may already be an admin."
                });
    ...
    public async Task<IActionResult> DemoteToMember(int id, int memberId)
    {
        try
        {
            ...
            if (!result)
                return BadRequest(new
                    { message = "Failed to demote admin. You may lack permissions or this is the only admin." });
    ...
    public async Task<IActionResult> RemoveMember(int id, int memberId)
    {
        try
        {
            ...
            if (!result)
                return BadRequest(new
                    { message = "Failed to remove member. You may lack permissions or cannot remove this user." });
    ...
    public async Task<IActionResult> TransferOwnership(int id, [FromBody] TransferOwnershipDto dto)
    {
        try
        {
            ...
            if (!result)
                return BadRequest(new
                {
                    message = "Failed to transfer ownership. You must be the owner and the target must be a member."
                });
